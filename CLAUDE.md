...
- Pushes to `main` trigger [release-please](.github/workflows/release-please.yml). The `release-please` job doesn't run the action directly — it calls the shared reusable workflow `sushidev-team/workflow-release-please/.github/workflows/release-please.yml@main`, which mints a GitHub App token from the org's release bot (secrets `RELEASE_BOT_APP_ID` / `RELEASE_BOT_PRIVATE_KEY`) and runs release-please as that bot instead of the default `github-actions` user. It opens/updates a release PR (bumps `CHANGELOG.md` + version).
- release-please runs in **manifest mode**: `.release-please-config.json` (release-type `go`, `include-v-in-tag`) and `.release-please-manifest.json` (last released version) must both exist and be committed — the reusable workflow always passes `config-file`/`manifest-file`.
