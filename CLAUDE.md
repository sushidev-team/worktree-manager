...
- Pushes to `main` trigger [release-please](.github/workflows/release-please.yml). The `release-please` job first mints a GitHub App token from the org's release bot via `actions/create-github-app-token` (secrets `RELEASE_BOT_APP_ID` / `RELEASE_BOT_PRIVATE_KEY`), then runs release-please with that token so the release PR/tag are authored by the bot, not the default `github-actions` user. It opens/updates a release PR (bumps `CHANGELOG.md` + version). (The bot step is inlined rather than calling the org's shared reusable workflow: that workflow's repo is private and a **public** repo like this one can't call a private/internal reusable workflow.)
- release-please runs in **manifest mode**: `.release-please-config.json` (release-type `go`, `include-v-in-tag`) and `.release-please-manifest.json` (last released version) must both exist and be committed — the job passes `config-file`/`manifest-file` to the action.
- Merging the release PR tags a release, then the `goreleaser` job (gated on the reusable workflow's `release_created` output) runs **GoReleaser** (`.goreleaser.yaml`), which builds cross-platform binaries, publishes the GitHub release, and updates the Homebrew tap (`sushidev-team/homebrew-tap`). GoReleaser still uses `GITHUB_TOKEN` + `HOMEBREW_TAP_TOKEN`, not the bot.
